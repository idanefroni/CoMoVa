...
getAllOrthologsByRE <- function(locus, orthologs, RE, orthologlist=names(orthologs), motifVarWeights=NULL) {
	# calculate variants weights
	## Sum the abundenace of each variants across all genomes. Normalize to 0-1 (divide by sum)
	## Then take the inverse of the ratio.
  
  if(is.null(motifVarWeights)) {
  
	  motifVarWeights =sapply(
						  rowSums(sapply(names(RE), function(x) { table(RE[[x]][,"Variant"])})) / 
						  sum( rowSums(sapply(names(RE), function(x) { table(RE[[x]][,"Variant"])}))), 
  							function(x) { 1/x })
  }

#  baseRE = sort(getREVariants("Athaliana", locus, RE)[,3])
  baseRE = getREVector(unique(getREVariants("Athaliana", locus, RE)[,"Variant"]), motifVarWeights)
...
#                bestRE = getREDist(baseRE, getREVariants(x , orth[1], RE)[,3])
                bestRE = getREDist(baseRE, getREVector(unique(getREVariants(x , orth[1], RE)[,"Variant"]),motifVarWeights)) 
                for(i in 2:length(orth)) {
                	newREDist = getREDist(baseRE, getREVector(unique(getREVariants(x , orth[i], RE)[,"Variant"]),motifVarWeights))
                  
                  if(newREDist < bestRE) {
                    #print(paste0("better ortholog: " , orth[i] , ". distance: ", bestRE))
...
getREVector <- function(RElist, REweights) {
	reVec = rep(0, length(REweights))
	names(reVec) = names(REweights)
	weightedre = table(RElist)*REweights[names(table(RElist))]
	reVec[names(weightedre)] = weightedre
	reVec
}
####### getREDist 
#
...
loadcontable <- function(name, promoterlength, background=NULL, directory="REDB") {
  
  con = list()
  con$angio = as.matrix(read.csv(paste0(directory,"/",name, "/angio_", promoterlength, ".csv"), as.is=TRUE, row.names=1)) 
  con$dicots = as.matrix(read.csv(paste0(directory,"/",name, "/dicots_", promoterlength, ".csv"), as.is=TRUE, row.names=1))
  con$monocots = as.matrix(read.csv(paste0(directory,"/",name, "/monocots_", promoterlength,".csv"), as.is=TRUE, row.names=1))
  con$bra = as.matrix(read.csv(paste0(directory,"/",name, "/bra_", promoterlength,".csv"), as.is=TRUE, row.names=1))
...
########### makePvalMatrix
#
# utility function that accepts a matrix of conservation scores, fits a negative binomial distribution to the background and returns a matrix of conservation pvalues
#
...
loadTree <- function(tree, gene, redb) {
  ...
  for(i in intersect(names(orth), names(tree[[2]]))) {
    tree[[2]][[i]]$variant = unique(getREVariants( i, orth[i], redb)[,"Variant"])
  }
...
getConservationData <- function(gene,redb) {
  if(!exists("variantDic")) {
    variantDic = sort(unique(redb[[1]][,"Variant"]))
  }
